...
public class OpenIddictDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    ...
    {
        if (await _openIddictScopeRepository.FindByNameAsync("FeatureRequestPortal") == null)
        {
            await _scopeManager.CreateAsync(new OpenIddictScopeDescriptor {
                Name = "FeatureRequestPortal", DisplayName = "FeatureRequestPortal API", Resources = { "FeatureRequestPortal" }
            });
        }
    }
    ...
    private async Task CreateApplicationsAsync()
    {
        ...
        //Web Client
        var webClientId = configurationSection["FeatureRequestPortal_Web:ClientId"];
        if (!webClientId.IsNullOrWhiteSpace())
        {
            var webClientRootUrl = configurationSection["FeatureRequestPortal_Web:RootUrl"]!.EnsureEndsWith('/');

            /* FeatureRequestPortal_Web client is only needed if you created a tiered
            ...
            await CreateApplicationAsync(
                ...
                secret: configurationSection["FeatureRequestPortal_Web:ClientSecret"] ?? "1q2w3e*",
